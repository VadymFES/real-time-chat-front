...
export default function AddMessage({ currentUser, selectedRoomId }: AddMessageProps): JSX.Element {
  const [message, setMessage] = useState("");
  const [socket, setSocket] = useState<WebSocket | null>(null);


  useEffect(() => {
    const newSocket = new WebSocket(`ws://51.20.108.68/mesasges/send_message/${selectedRoomId}`); 
